<template>
	<!-- 添加定时任务 按钮 -->
	<el-button @click="dialogCron = true; updateBtn = false" type="success" icon="Plus" size="small"
	...
	<el-dialog v-model="dialogCron" width="45%">
		...
		<el-form :model="cronTabData" ref="cronTabData" label-width="80px">
			...
			<el-form-item label="执行计划">
				<el-select v-model="cronTabData.plan" placeholder="请选择计划" style="width: 100%">
					<el-option v-for="item in testEnvs" :key="item.id" :label="item.name" :value="item.id"></el-option>
				</el-select>
			</el-form-item>
...
export default {
	...
	methods: {
		...
		// 添加定时任务
		async createCron() {
			// const isValid = this.$refs.cronTabData.validate();
			// if (!isValid) {
			// 	this.showMessageBox('error', '表单验证未通过，请填写必填项', 1000);
			// 	return;
			// }
			...
			console.log("创建定时任务接口参数params : ", params);
